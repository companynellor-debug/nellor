...
const Patrocinios = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "scheduled": return <Badge className="bg-blue-500 gap-1"><Calendar className="h-3 w-3" />Agendado</Badge>;
    ...
    <div className="space-y-6">
              ...
              <Card key={req.id} className="p-5">
                <div className="flex flex-col md:flex-row gap-4">
                  {req.banner_image_url && (
                    <div className="w-full md:w-48 h-32 rounded-lg overflow-hidden bg-muted cursor-pointer flex-shrink-0"
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    {req.product_name && (
                      <p className="text-sm"><span className="text-muted-foreground">Produto:</span> <span className="font-medium">{req.product_name}</span></p>
                    )}
                    {req.message && (
                      <p className="text-sm text-muted-foreground bg-muted/50 p-3 rounded-lg">"{req.message}"</p>
                    )}
                    <p className="text-xs text-muted-foreground">
                      Enviado em {new Date(req.created_at).toLocaleDateString("pt-BR")} às {new Date(req.created_at).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}
                    </p>
                  </div>
                  <div className="flex md:flex-col gap-2 flex-shrink-0">
                    ...
                    {req.type === "banner_homepage" && (
                      <Button size="sm" variant="outline" className="gap-1"
                        onClick={() => setActionModal({ open: true, request: req, action: "schedule" })}>
              ...
              <Card key={req.id} className="p-4">
                <div className="flex items-center justify-between gap-4">
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    {getTypeBadge(req.type)}
                    {req.product_name && (
                      <span className="text-sm text-muted-foreground truncate hidden sm:inline">{req.product_name}</span>
                    )}
                  </div>
                  <div className="flex items-center gap-2">
                    {req.scheduled_date && (
                      <span className="text-xs text-muted-foreground">{new Date(req.scheduled_date).toLocaleDateString("pt-BR")}</span>
                    )}
                    {getStatusBadge(req.status)}
                  </div>
                </div>
                {req.admin_response && (
                  <p className="text-xs text-muted-foreground mt-2 pl-7">Resposta: {req.admin_response}</p>
                )}
              </Card>
            ))}
          </div>
      ...
      <Dialog open={actionModal.open} onOpenChange={(open) => !open && setActionModal({ open: false, request: null, action: null })}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {actionModal.action === "approve" ? "Aprovar Patrocínio"
              : actionModal.action === "reject" ? "Rejeitar Patrocínio"
              : "Agendar Patrocínio"}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div>
              <Label>{actionModal.action === "reject" ? "Motivo da Rejeição *" : "Mensagem para o Fornecedor (opcional)"}</Label>
              <Textarea value={adminResponse} onChange={(e) => setAdminResponse(e.target.value)}
                placeholder={actionModal.action === "reject" ? "Explique o motivo da rejeição..." : "Adicione uma observação se necessário..."} rows={3} />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setActionModal({ open: false, request: null, action: null })}>Cancelar</Button>
            <Button onClick={handleAction} disabled={submitting}
              className={actionModal.action === "approve" ? "bg-green-600 hover:bg-green-700" : ""}
      ...
      <Dialog open={!!previewImage} onOpenChange={() => setPreviewImage(null)}>
        <DialogContent className="max-w-3xl">
          <DialogHeader><DialogTitle>Preview do Banner</DialogTitle></DialogHeader>
          {previewImage && <img src={previewImage} alt="Banner preview" className="w-full rounded-lg" />}
        </DialogContent>
      </Dialog>
    </div>
  );
