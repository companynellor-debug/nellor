...
const Pedidos = () => {
  ...
  // Status do repasse Stripe
  const getTransferStatus = (order: Order) => {
    // Pagamento via Stripe = repasse automático
    ...
    // Pagamento PIX/Boleto = repasse manual pendente
                ...
                <div key={order.id} className="p-3 sm:p-4 md:p-6 hover:bg-muted/20 transition-colors">
                  ...
                  <div className="bg-muted/30 rounded-lg p-2 mb-3 text-xs">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[10px] sm:text-xs">
                      ...
                      <div>
                        <span className="text-muted-foreground">{order.payment_method === 'cartao' ? 'Taxa Stripe:' : 'Taxa:'}</span>
                        <p className="font-medium text-orange-600">-R$ {breakdown.taxaStripe.toFixed(2)}</p>
                      </div>
                      <div>
                        <span className="text-muted-foreground">Líquido:</span>
                        <p className="font-semibold text-green-600">R$ {breakdown.valorLiquido.toFixed(2)}</p>
                      </div>
                    </div>
                  </div>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-medium">{(selectedOrder.endereco_entrega as any)?.name}</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Documento</p>
                    <p className="font-medium">{(selectedOrder.endereco_entrega as any)?.document}</p>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <p className="text-sm">
                  ...
                  <br />
                  {(selectedOrder.endereco_entrega as any)?.neighborhood} - {(selectedOrder.endereco_entrega as any)?.city}/{(selectedOrder.endereco_entrega as any)?.state}
                  <br />
                  CEP: {(selectedOrder.endereco_entrega as any)?.zip_code}
                </p>
              </div>
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                  {((selectedOrder.itens as any)?.tags || []).map((tag: string) => (
                    ...
                    >
                      {tag}
                      <button
                        onClick={() => handleRemoveTag(selectedOrder.id, tag)}
                ...
                <div className="flex gap-2">
                  {predefinedTags.map(tag => (
                    <Button
                      ...
                      size="sm"
                      variant="outline"
                      onClick={() => handleAddTag(selectedOrder.id, tag.label)}
                    >
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <Button
                    ...
                    disabled={selectedOrder.order_status === 'preparing'}
                  >
