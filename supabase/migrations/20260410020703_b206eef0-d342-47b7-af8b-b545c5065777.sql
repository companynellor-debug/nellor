...
BEGIN
  ...
  IF NEW.order_status = 'cancelled' THEN
    ...
    -- Buyer can only cancel if order is still pending (supplier hasn't accepted yet)
    IF v_uid = OLD.buyer_id THEN
      IF OLD.order_status != 'pending' THEN
        RAISE EXCEPTION 'Você só pode cancelar pedidos que ainda estão pendentes. Entre em contato com o fornecedor.';
      END IF;
      RETURN NEW;
    END IF;

    -- Supplier can only cancel if payment is still pending
    IF v_uid = OLD.supplier_id THEN
      IF OLD.payment_status != 'pending' THEN
        RAISE EXCEPTION 'Não é possível cancelar um pedido já pago. Solicite um reembolso ao administrador.';
      END IF;
      RETURN NEW;
    END IF;
