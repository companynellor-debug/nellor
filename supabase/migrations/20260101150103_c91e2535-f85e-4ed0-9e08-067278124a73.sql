-- Update track_affiliate_click to also return product_id
CREATE OR REPLACE FUNCTION public.track_affiliate_click(_code text, _buyer_id uuid DEFAULT NULL::uuid, _visitor_id text DEFAULT NULL::text, _user_agent text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  v_existing_id uuid;
BEGIN
  SELECT id, affiliate_id, supplier_id, product_id, clicks
  INTO v_link
